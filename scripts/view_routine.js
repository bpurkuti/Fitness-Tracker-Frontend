...
async function getRoutineExercises() {
	let config = {
		method: "POST",
		headers: { "Content-Type": "application/json" },
		body: JSON.stringify({ session: sessionStorage.getItem("session"), routineId: sessionStorage.getItem("currRoutineId") })
	};
	...
	if (response.status === 200) {
		...
		for (let i = 0; i < routineExercises.length; i++) {
			...
			if (routineExercise.duration !== 0 || routineExercise.weight !== 0 || routineExercise.reps !== 0) {
		...
		for (let i = 0; i < routineExercises.length; i++) {
			let routineExercise = routineExercises[i];
			if ((routineExercise.duration === 0 && routineExercise.weight === 0 && routineExercise.reps === 0) && activeExerciseSet === -1) {
		...
		for (let i = 0; i < routineExercises.length; i++) {
			let routineExercise = routineExercises[i];
			if (routineExercise.duration === 0 && routineExercise.weight === 0 && routineExercise.reps === 0 && activeExerciseSet !== i) {
				let html = `<div class="vforminner" id="${count}" value=${routineExercise.routineExerciseId}>`;
				html += `<h5 class="blocktitle">${routineExercise.exerciseName}</h5>`;
				html += `<h5 class="status">Upcomming</h5>`;
				html += '</div>';
				routineBlock.innerHTML += html;
				newList.push(routineExercise);
				count++;
